...
def verify_statistical_tests_mf0_0004():
    ...
    for i, line in enumerate(lines):
        if 'NEWEY-WEST HAC' in line or 'Newey-West HAC' in line:
            # The p-value is on the next line
            if i+1 < len(lines):
                next_line = lines[i+1]
                match = re.search(r'p-value=([0-9.]+)', next_line)
                if match:
...
def verify_walk_forward_includes_all_mf():
    ...
    if 'MF candidates: [0.0002, 0.0003, 0.0004, 0.0005]' in content:
        ...
        # Additionally, check that there is variation in best MF per window (optional)
        # We'll just check that there are multiple different MFs in the Best MF column
        lines = content.split('\n')
        best_mfs = set()
        for line in lines:
...
def verify_readme_mf0_0004_numbers():
    ...
    with open(readme_file, 'r', encoding='utf-8') as f:
        content = f.read()
    # Check for the key numbers in the Results section for MF=0.0004
    # We'll look for the string "MF=0.0004" and then nearby numbers
    # Simpler: check for the expected trades/month, net PnL, Sharpe, max DD
    expected = {
        'Trades/month': '25.22',
        'Net PnL': '22.19%',
        'Sharpe': '2.58',
