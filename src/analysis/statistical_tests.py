...
sr = mean_ret / std_ret * ann_factor if std_ret != 0 else 0
...
expected_max = ((1 - gamma) * norm.ppf(1 - 1/n_trials) +
                gamma * norm.ppf(1 - 1/(n_trials * np.e)))
# Standard deviation of the Sharpe ratio estimator
sr_std = np.sqrt((1 + 0.5*sr**2 - skew_val*sr + (kurt_val-3)/4*sr**2) / (n_trades - 1))
# Deflated Sharpe Ratio
dsr = (sr - expected_max * sr_std) / sr_std
...
print(f"  Expected maximum: {expected_max:.4f}")
print(f"  DSR: {dsr:.4f}")
...
with open(output_file, 'w') as f:
    ...
    f.write(f"Skew: {skew_val:.4f}\n")
    f.write(f"Kurtosis (excess): {kurt_val:.4f}\n")
    f.write(f"Sharpe ratio (sr): {sr:.4f}\n")
    f.write(f"Sharpe ratio std (sr_std): {sr_std:.4f}\n")
    f.write(f"Expected maximum: {expected_max:.4f}\n")
